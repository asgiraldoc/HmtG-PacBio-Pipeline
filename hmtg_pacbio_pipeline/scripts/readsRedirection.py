...
def primerDetection(input_file, primerF, primerR):
    ...
    # If the dictionary contains more than 10000 sequences, randomly select 10000 sequences
    if len(seq_dict) > 10000:
        # seq_dict = dict(random.sample(seq_dict.items(), 10000))
    ...
    with open(outf + "_nolong.fasta", "w") as f, open(outf + "_long.fasta", "w") as g:
